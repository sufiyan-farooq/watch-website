...
var search = document.getElementById("brand_products");
var searchBrand = document.getElementById("search_brand");

function addProducts(event) {
  var brandValue = event.target.value;
  search.innerHTML = `
    <option selected disabled value="">Please Select Product</option>
    
    `;
  for (var products in watches[brandValue]) {
    search.innerHTML += `
            <option  value="${watches[brandValue][products].name}" >${watches[brandValue][products].name}</option>
        `;
  }
  return brandValue
}
...
// check the selected product on website

function findProduct(event, brandValue) {
  var productName = event.target.value;
  for (var model in watches[productName]) {
  }
}
...
for (var key in watches) {
  // console.log(watches[key]);

  for (var products in watches[key]) {
    var productName = watches[key][products];
    space.innerHTML += `
    <div  class="wrapper">
    <div class="container">
      ...
      <div class="bottom">
        <div class="left">
          <div class="details">
            <h1 style="font-size: 20px">${productName.name}</h1>
    ...
    <div class="inside">
      <div class="icon"><i class="material-icons">Detail</i></div>
      <div class="contents">
        <div>
          <h2>${productName.category}</h2>
        </div>
        <div>
           <h6>${productName.fullName}</h6>
        </div>
        <div>
          <h6> Color We Have : ${productName.color}  </h6>
        </div>
        <div>
        <h6>Tags : ${productName.Tags}</h6>
        </div>
        <div>
        <h4>PRICE : ${productName.price}</h4>
        </div>
  ...
  if(watches[brand][key].name === product) {
    ...
    <div class="product-detail">
     <div class="back-button">
       <a href="main.html"><button>x</button>
       </a>
     </div>
    
     <div class="product-name">
       <h1>${watches[brand][key].name}</h1>
     </div>
     <div class="product-fullName">
       <h4>${watches[brand][key].fullName}</h4>
     </div>
     <div class="product-caption">
       <p> ${watches[brand][key].caption}</p>
     </div>
